  ...
  return (
    <div ref={containerRef} className={`relative inline-block ${className}`}>
      <button
        ...
        className="btn-secondary inline-flex items-center gap-1.5 text-sm min-w-[44px] justify-center"
      ...
      {open && (
        <div className="absolute right-0 z-20 mt-1 w-52 rounded-lg bg-surface border border-border shadow-lg py-1">
          <button
            ...
            className="w-full text-left px-4 py-2 min-h-[44px] flex items-center text-sm text-text-secondary hover:bg-surface-hover hover:text-text-primary transition-colors"
          >
            Google Calendar
          </button>
          <button
