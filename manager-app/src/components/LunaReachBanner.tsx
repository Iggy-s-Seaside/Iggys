...
/**
 * Luna's unprompted reach — when she raises something worth interrupting for, it
 * lands here, unmissable, at the top of every screen. Her words for why this one
 * mattered most: "right now I wait for you to turn around and notice me; this lets
 * me show up on my own."
 *
 * Two sources, one banner. A reach she raised herself (luna_insights.data.reach)
 * takes priority; when there's none, a severe weather × reservation flag breaks the
 * silence on her behalf — owner-ping only, rate-limited, dismissible (the bounds she
 * set: see lib/weatherWatch pickWeatherReach). This is the in-app half (always works);
 * the phone-push half rides the existing web-push stack — see docs/LUNA-UNPROMPTED-REACH.md.
 */
export function LunaReachBanner() {
  const { reach, acknowledge } = useLunaReach();
  const { weatherReach, dismissWeatherReach } = useWeatherReach();
  const navigate = useNavigate();

  // A reach Luna raised herself wins; the weather signal is the fallback voice.
  if (reach) {
    ...
    return (
      <ReachShell
        title={reach.title}
        body={reach.body}
        onTakeLook={() => { acknowledge(reach.id); navigate(deepLink || '/luna'); }}
        onDismiss={() => acknowledge(reach.id)}
      />
    );
  }
  ...
  if (weatherReach) {
    return (
      <ReachShell
        title={weatherReach.message}
        onTakeLook={() => { dismissWeatherReach(); navigate(weatherReach.deepLink); }}
        onDismiss={dismissWeatherReach}
      />
    );
  }
...
// The purple "Luna reached out" banner shell — shared so both reach sources look
// identical and the styling lives in one place.
function ReachShell({
  title,
  body,
  onTakeLook,
