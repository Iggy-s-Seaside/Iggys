...
- 2026-06-14 — ADVERSARIAL REVIEW PASS + 6 FIXES SHIPPED+DEPLOYED (commit dc0c9de). 10-agent review (per-area finders + adversarial verifiers) audited the wave-5 sweep + Stripe + new features. 6 CONFIRMED real issues, all fixed: (CRITICAL) Pipeline drag-to-Paid fabricated payment_status='partial' with no money → self-contradictory $0-paid 'Partial' badge + 409-blocked the real Stripe deposit link; (HIGH) back-drag from Paid wiped a genuinely-paid party to 'unpaid'; FIX = board never writes payment_status, Paid moves route to the deposit panel. (HIGH) usePayments.requestCheckoutLink missing success_url/cancel_url → manager "Request deposit" 400'd every time; FIXED. (HIGH) merch webhook left paid orders with zero line items after a partial-failure retry; FIX = backfill items, only dup once items exist. (MEDIUM) party_deposit webhook overwrote amount_paid + never recomputed balance_due; FIX = additive + invariant-preserving + flip to 'paid' when covered. (MEDIUM) outbox flush single-flight guard (no concurrent double-replay). Manager build green; stripe-webhook redeployed; manager front-end redeployed; Pipeline smoke-checked live. SPRINT COMPLETE — waves 1-5 + Stripe(test) + review, all shipped/deployed/verified.
