...
const triggerClasses = (variant: 'glass' | 'manager', className?: string) =>
  [
    ...
    variant === 'manager'
      ? 'rounded-lg bg-surface border border-border px-3 py-2 min-h-[42px] text-text-primary hover:border-primary/40'
...
function itemClasses(active: boolean, selected: boolean, disabled?: boolean) {
  ...
  if (disabled) return `${base} text-text-dim opacity-40 line-through cursor-not-allowed`;
...
function HintChip({ hint }: { hint: string }) {
  return <span className={`shrink-0 text-[11px] ${hint === 'next day' ? 'text-accent' : 'text-text-dim'}`}>{hint}</span>;
}
  ...
  return (
    <div className="relative">
      ...
      <select
        ...
        className={`${triggerClasses(variant, className)} appearance-none pr-10 ${LeadingIcon ? 'pl-10' : ''} ${selected ? '' : 'text-text-dim'}`}
      >
  ...
  return (
    <>
      ...
      >
        ...
        <span className={`flex-1 truncate ${selected ? '' : 'text-text-dim'}`}>{selected ? selected.label : placeholder}</span>
                ...
                {options.map((opt, i) => {
                  ...
                  return (
                    <Fragment key={String(opt.value)}>
                      {showHeader && (
                        <div className="px-3 pt-2 pb-1 text-[11px] uppercase tracking-wider text-text-dim select-none">{opt.group}</div>
                      )}
                      <div
                        role="option"
                        aria-selected={isSel}
