...
const FEATURED_COCKTAILS: { name: string; shortDesc: string; dbMatch: string }[] = [
  { name: 'Marionberry Mule', shortDesc: 'Marionberry vodka, ginger beer, fresh lime', dbMatch: 'marionberry mule' },
  { name: 'Burlini Espresso Martini', shortDesc: 'Vodka, Kahlua, local espresso, chocolate rim', dbMatch: 'burlini espresso martini' },
  { name: "Iggy's Old Fashioned", shortDesc: 'Bulleit rye, muddled orange & cherry, bitters', dbMatch: 'old fashion' },
  { name: 'Key Lime Pie Martini', shortDesc: 'Bumbu rum, vanilla vodka, lime, cream, pie crust rim', dbMatch: 'key lime pie' },
];
...
const draftBeers = [
  { name: 'Coastal Haze', brewery: 'Public Coast', style: 'Hazy IPA', abv: '7.2%', price: '$7' },
  { name: 'Czech Pilsner', brewery: 'Buoy Beer', style: 'Pilsner', abv: '6.2%', price: '$7' },
  { name: 'Vortex IPA', brewery: 'Fort George', style: 'IPA', abv: '7.2%', price: '$7' },
  { name: 'Kona Big Wave', brewery: 'Kona Brewing', style: 'Blonde Ale', abv: '4.4%', price: '$7' },
];
...
export default function Home() {
  ...
  const cocktailItems = FEATURED_COCKTAILS.map((featured) => {
    const match = allCocktails.find(
      (c) => c.name.trim().toLowerCase().includes(featured.dbMatch)
    );
    const price = match?.price?.startsWith('$') ? match.price : match ? `$${match.price}` : '';
        ...
        <div className="relative z-10 w-full section-container pb-20 pt-40">
          <div className="max-w-2xl">
            ...
            <img
              ...
              style={{
                filter:
                  'drop-shadow(0 0 2px rgba(255,255,255,0.9)) drop-shadow(0 0 5px rgba(255,255,255,0.5)) drop-shadow(0 0 15px rgba(45,212,191,0.6)) drop-shadow(0 0 30px rgba(45,212,191,0.3))',
              }}
            />
            <h1 className="sr-only">Iggy's Seaside Bar</h1>

            {/* Location in script font with amber accent */}
            <p className="font-display text-2xl md:text-3xl text-accent mb-4"
            ...
            >
              Handcrafted cocktails, local craft beers &amp; fresh Pacific
              Northwest seafood.
            </p>
        ...
        <div className="section-container">
          <div className="flex flex-wrap items-center justify-center gap-4 md:gap-6">
            ...
            <div className="flex flex-wrap gap-2">
              {['$5 Draft Beer', '$5 Well Drinks', '$3 Canned Beer', 'Food Specials'].map(
                (deal) => (
                  <span
                    key={deal}
            ...
            <Link to="/happy-hour" className="btn-primary text-sm px-4 py-2">
              See All Deals
            </Link>
          </div>
        </div>
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6 mt-12">
            {features.map((feature) => (
              <div key={feature.title} className="glass-card-hover p-6">
                ...
                <p className="text-text-muted text-sm mt-2">{feature.description}</p>
                <Link
                  to={feature.link}
                    ...
                    return (
                      <div key={event.id} className="glass-card-hover p-4 flex gap-4">
                        ...
                        <div className="min-w-0">
                          <h4 className="font-heading font-semibold text-white">{event.title}</h4>
                          <p className="text-text-muted text-sm">{event.time}</p>
                          {event.is_recurring && (
                            <span className="text-accent text-xs">Every {event.recurring_day}</span>
                          )}
                        </div>
                      </div>
                    );
              ...
              {activeSpecials.length > 0 && (
                <div className="space-y-4">
                  ...
                  {activeSpecials.map((special) => (
                    <div key={special.id} className="glass-card-hover p-4 flex items-start justify-between gap-3">
                      <div className="min-w-0">
                        <h4 className="font-heading font-semibold text-white">{special.title}</h4>
                        <p className="text-text-muted text-sm mt-1 line-clamp-2">{special.description}</p>
                      </div>
                      {special.price && (
                        <span className="text-primary font-bold text-lg shrink-0">{special.price}</span>
                      )}
                    </div>
                  ))}
                </div>
      ...
      <section className="section-padding bg-surface/50">
        ...
        >
          <div className="grid md:grid-cols-2 gap-12 items-center">
            ...
            <div>
              <div>
                <p className="font-display text-accent text-xl mb-1">Crafted with care</p>
                ...
                <p className="text-text-muted mt-4 max-w-2xl">
                  Our bartenders don't just pour drinks — they craft experiences. Every cocktail on our menu is designed to surprise and delight.
                </p>
              </div>
                ...
                {cocktailItems.map((item) => (
                  <div key={item.name} className="flex items-start gap-3">
                    ...
                    <div>
                      ...
                      <span className="text-primary ml-2 text-sm font-semibold">{item.price}</span>
      ...
      <section className="relative py-24 lg:py-32 overflow-hidden">
        ...
        >
          ...
          <p className="text-text-muted max-w-xl text-lg">
            Covered patio upstairs, fire pit outside, indoor bar. Dogs welcome. Plenty of TVs for the game. Pick your vibe.
          </p>
      ...
      <section className="section-padding bg-surface/50">
        ...
        >
          <div className="flex flex-col sm:flex-row justify-between items-start sm:items-end gap-6 mb-10">
            ...
            />
            <Link to="/beers" className="btn-outline text-sm shrink-0">
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4">
            {draftBeers.map((beer) => (
              <div key={beer.name} className="glass-card-hover p-6">
                ...
                <h3 className="font-heading text-lg font-semibold text-white">{beer.name}</h3>
                <p className="text-text-muted text-sm mt-1">{beer.brewery}</p>
                <p className="text-primary font-bold mt-3 text-lg">{beer.price}</p>
              </div>
            ))}
          </div>
        </div>
